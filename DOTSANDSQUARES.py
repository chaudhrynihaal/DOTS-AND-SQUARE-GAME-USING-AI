...
def minimax(cells, depth, alpha, beta, maximizingPlayer):
   
    if depth == 0:
        return None, evaluate(cells)
        ...
        for cell in cells:
            if not cell.winner:
                for i in range(4):
                    if not cell.sides[i]:
                        cell.sides[i] = True
                        _, eval = minimax(cells, depth - 1, alpha, beta, False)
                        cell.sides[i] = False
                        if eval > maxEval:
                            maxEval = eval
                            bestMove = cell
        ...
        for cell in cells:
            if not cell.winner:
                for i in range(4):
                    if not cell.sides[i]:
                        cell.sides[i] = True
                        _, eval = minimax(cells, depth - 1, alpha, beta, True)
                        cell.sides[i] = False
                        if eval < minEval:
                            minEval = eval
                            bestMove = cell
...
def evaluate(cells):
    # Simple evaluation function, you can improve it based on your game's strategy
    p1_score = sum(1 for cell in cells if cell.winner == 'X')
    p2_score = sum(1 for cell in cells if cell.winner == 'O')
...
while running:
    ...
    if player == 'O' and not gameover:
        best_move, _ = minimax(cells, depth=2, alpha=-math.inf, beta=math.inf, maximizingPlayer=True)
        if best_move:
            index = best_move.index
            for i in range(4):
                if not best_move.sides[i]:
                    best_move.sides[i] = True
